...
// 查询数据库以获取统计数据
$sql = "SELECT COUNT(*) AS totalProducts, AVG(pp_price) AS avgPrice FROM product";
...
// 查询数据库以获取每个商品的点击数
$sqlClicks = "SELECT pp_name, pp_total_soldnum FROM product";
$resultClicks = $conn->query($sqlClicks);

// 计算所有商品的点击数总和
$totalClicks = 0;
while ($rowClicks = $resultClicks->fetch_assoc()) {
  $totalClicks += $rowClicks['pp_total_soldnum'];
}
...
if ($resultClicks->num_rows > 0) {
  while ($rowClicks = $resultClicks->fetch_assoc()) {
    ...
    $clicks = $rowClicks['pp_total_soldnum'];
...
// 查询数据库以获取点击数和加入购物车数
$sqlClicksAndCarts = "SELECT pp_name, pp_total_soldnum, pp_carts, pp_orders FROM product";
...
if ($resultClicksAndCarts) {
  ...
  while ($rowClicksAndCarts = $resultClicksAndCarts->fetch_assoc()) {
    $totalClicks += $rowClicksAndCarts['pp_total_soldnum'];
  ...
  if ($resultClicksAndCarts->num_rows > 0) {
    ...
    while ($rowClicksAndCarts = $resultClicksAndCarts->fetch_assoc()) {
      ...
      $clicks = $rowClicksAndCarts['pp_total_soldnum'];
