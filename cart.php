...
$conn = new mysqli("localhost", "root", "", "petstore"); // 修改資料庫名稱為 'customer'
...
if (!isset($_SESSION['username'])) {
  header("refresh:3; url=login.php"); // 跳轉到登錄頁面
  echo "用户未登录，将在3秒后跳转到登录页面。";
  exit;
}

$username = $_SESSION['username'];

$sql = "SELECT c_id FROM customer WHERE c_mail = '$username'";
...
if ($result->num_rows > 0) {
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "UPDATE customer SET c_cart = '$cartData' WHERE c_id = '$userID'"; // 修改為 'customer' 資料表
    $conn->query($sql);
  }
...
<body>
    ...
    <?php
        ...
        if (!empty($cartData)) {
          ...
          foreach ($cartData as $SKU => $quantity) {
            ...
            $sql = "SELECT * FROM product WHERE pp_SKU = '$SKU'";
            $result = $conn->query($sql);
            if ($result->num_rows > 0) {
              $row = $result->fetch_assoc();
              ...
              echo "<td><a href='serve.php?remove={$SKU}'>刪除</a></td>"; // 修改連結
              echo "</tr>";
            }
          }
        } else {
  ...
  <script>
    setTimeout(function() {
      window.location.href = "login.php"; // 跳轉到登錄頁面
    }, 3000);
  </script>
</body>
</html>
